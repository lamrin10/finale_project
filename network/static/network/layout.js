...
function follow_user(element, username, origin) {
    ...
    .then(() => {
        if(origin === 'suggestion') {
            element.parentElement.innerHTML = `<button class="btn btn-info" type="button" onclick="unfollow_user(this,'${username}','suggestion')">Following</button>`;
        }
       
        else if(origin === 'edit_page') {
            element.parentElement.innerHTML = `<button class="btn btn-info float-right" onclick="unfollow_user(this,'${username}','edit_page')" id="following-btn">Following</button>`;
        }
    ...
    eachrow.innerHTML = `
        ...
        <div style="flex: 1;">
            <div class="comment-text-div">
               ...
               <div> ${comment.body}<span style ="font-size:10px;color:red">${comment.sentiment}</span></div>
            </div>
        </div>`;
    if (new_comment) {
...
function search(event) {
    ...
    if (query !== "") {
        ...
        // Clear and hide the suggestions when the search box is empty
        clearSuggestions();
    }
}

function displaySuggestions(suggestions) {
    // Implement logic to display suggestions in your UI
    // For example, you can update a dropdown or list with the suggestions
}

function clearSuggestions() {
    // Implement logic to clear and hide the suggestions in your UI
}
...
function search(event) {
    ...
    // Perform an AJAX request to the server-side endpoint (replace with your actual endpoint)
    // For simplicity, we'll use the fetch API here
    fetch("/search?query=" + encodeURIComponent(query))//no backend handler made for search
        .then(response => response.json())
        .then(data => {
            console.log("Received data:", data);
            // Handle the received data and update the searchResults div
            displayResults(data);
...
function displayResults(data) {
    ...
    if (data && data.results) {
        data.results.forEach(result => {
            ...
            resultElement.textContent = result.title; // Replace with the appropriate property of your result object
...
function displayResults(data) {
    ...
    if (data && data.results) {
        data.results.forEach(result => {
            // Customize how each user result is displayed
            var resultElement = document.createElement("div");
            
            // Assuming your User model has properties like 'username', 'profile_pic', 'first_name', etc.
            resultElement.innerHTML = `
                <div>
                    <img src="${result.profile_pic}" alt="${result.username}" />
                </div>
                <div>
